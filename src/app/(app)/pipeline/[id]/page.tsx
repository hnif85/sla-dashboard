    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href="/pipeline" className="text-gray-400 hover:text-gray-600">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-gray-900">{prospect.namaProspek}</h1>
      ...
      {tab === "Detail" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h2 className="font-semibold text-gray-900 mb-4">Informasi Prospek</h2>
            <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
              ...
              <div className="col-span-2">
                ...
                <div className="text-gray-900">{prospect.nextAction || "-"}</div>
              </div>
              {prospect.reasonLost && (
                <div className="col-span-2">
                  <div className="text-gray-400 text-xs mb-0.5">Reason Lost</div>
                  <div className="text-red-700 bg-red-50 px-3 py-2 rounded-lg">{prospect.reasonLost}</div>
                </div>
              )}
              {prospect.linkDokumen && (
                <div className="col-span-2">
                  <div className="text-gray-400 text-xs mb-0.5">Link Dokumen</div>
                  <a href={prospect.linkDokumen} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline break-all">
                    {prospect.linkDokumen}
                  </a>
                </div>
              )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Est. UMKM</span>
                  <span className="font-semibold">{prospect.estUmkmReach?.toLocaleString("id-ID") || "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Nilai Deal</span>
                  <span className="font-semibold">Rp {prospect.estNilaiDeal?.toLocaleString("id-ID") || "-"}</span>
                </div>
                <div className="flex justify-between border-t pt-2">
                  <span className="text-gray-500">Probability</span>
                  <span className="font-semibold text-green-600">
                    {prospect.probability != null ? `${(prospect.probability * 100).toFixed(0)}%` : "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Weighted UMKM</span>
                  <span className="font-bold text-blue-600">{Math.round(prospect.weightedUmkm || 0).toLocaleString("id-ID")}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Weighted Nilai</span>
                  <span className="font-bold text-blue-600">Rp {Math.round(prospect.weightedNilai || 0).toLocaleString("id-ID")}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                <span>MOM tersimpan</span>
                <span>{prospect.moms.length}</span>
              </div>
              <div className="flex justify-between text-xs text-gray-500">
                <span>Aktivitas</span>
                <span>{prospect.activities.length}</span>
              </div>
            </div>
          </div>
        </div>
